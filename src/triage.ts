...
function parseIncrementalReflection(raw: string): { sessionMemory?: string; promoted?: any[]; identity?: any; preferences?: any[] } | null {
  let s = (raw || '').trim();
  const fence = s.match(/```(?:json)?\s*([\s\S]*?)```/);
  if (fence) s = fence[1].trim();
...
${lines}
    ...
    if (idResult.rejected.length > 0 || idResult.applied > 0 || prefCount > 0) {
