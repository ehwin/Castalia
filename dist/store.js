...
import { isMemType, isClosedMemType, normalizeMarkdown, normalizeMemType } from './memType.js';
let saveCount = 0;
...
export async function saveMemory(params) {
    ...
    else if (params.autoClassify === true && params.text) {
        try {
...
export function promoteToProject(project, items, characterId) {
    ...
    for (const item of items || []) {
        ...
        const homeNames = { airi: ['AIRI'], lobehub: ['LobeHub'], shushu: ['AIRI', 'Castalia', 'LobeHub'], hermes: ['Hermes'], default: [] };
      ...
      `).all(proj, mt);
            const norm = (x) => (x || '').toLowerCase().replace(/[^\u4e00-\u9fa5a-z0-9]+/g, '');
            const bigrams = (x) => { const n = norm(x); const out = new Set(); for (let q = 0; q + 2 <= n.length; q++)
