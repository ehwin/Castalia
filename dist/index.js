...
import { DatabaseManager, listProjectNames, listMemTypeDirs, safeFilePart, sweepExpiredSessionMemories } from './db.js';
...
const TOOL_GROUPS = {
    ...
    admin: ['memory_list', 'stats_get', 'recent_conversations', 'daily_summary_data', 'reflect_analyze', 'reflect_apply', 'memory_context', 'context_get', 'project_list', 'project_create', 'instruction_list', 'instruction_delete', 'consolidate_deep'],
};
...
register('memory_search', 'agent', 'Search past memories using tag-first then vector KNN fallback. Use when recalling past events, facts, or user preferences.', {
    ...
    try {
        ...
        const r = await searchMemory({ ...base, profile: 'balanced', project: args.project });
        return ok({
            op: 'search',
            query: args.query,
            count: r.length,
...
register('reflect_batch_embed', 'harness', '[Internal] Batch embed all pending (un-embedded) memories. Called after reflect. characterId="any" embeds across all roles (e.g. reflect library).', { project: z.string().optional().describe('Project namespace (default: CASTALIA_PROJECT env or "default")'),
    characterId: z.string().optional().describe('Character id filter (default CHAR_ID; "any" = no filter, e.g. for reflect library)') }, async (args) => {
    try {
...
register('project_create', 'admin', 'Create a new project (library) namespace. Immediately usable for memory_save / memory_search with project=<name>. Name uses letters/digits/._- only.', {
    name: z.string().min(1).describe('New library name (letters/digits/._- recommended)'),
}, async (args) => {
    try {
        const raw = String(args.name).trim();
        if (!raw)
            return err('项目名不能为空', 'INVALID_PROJECT_NAME');
